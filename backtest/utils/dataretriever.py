...
class DataRetriever:
    def __init__(
        ...
        if (
            ...
            self.duration is None and self.start_date is None and self.end_date is None
        ):
            raise ValueError(
                "Please provide either duration or start_date and end_date"
            )
    ...
    def get_risk_free(
        self, ticker_index: pd.DatetimeIndex, rate_column: str = "DTB3"
    ) -> pd.DataFrame:
