...
def calculate_ReturnAnn(results_df: pd.DataFrame):
    ...
    total_return = (
        calculate_ReturnPct(results_df) / 100
...
def calculate_AvgTradeDuration(results_df: pd.DataFrame):
    ...
    return (
        pd.Timedelta(sum(durations, pd.Timedelta(0)) / len(durations))
        if durations
        else pd.Timedelta(0)
    )
...
def calculate_Expectancy(results_df: pd.DataFrame):
    ...
    avg_win = (
        np.mean([x for x in trade_returns if x > 0])
        if any(x > 0 for x in trade_returns)
        else 0
    )
    avg_loss = (
        abs(np.mean([x for x in trade_returns if x < 0]))
        if any(x < 0 for x in trade_returns)
        else 0
    )
...
def calculate_SQN(results_df: pd.DataFrame):
    ...
    return (np.mean(trade_returns) * np.sqrt(len(trade_returns))) / np.std(
        trade_returns
    )
